function getlast60s_avgch(filename, tetrode1_chans, tetrode2_chans)
    ...
    for t = 1:2
        ...
        % Get last 60s from each channel
%         signal_length = size(data, 1);  % total number of samples
%         last_60s_idx = (signal_length - 60 * fs + 1):signal_length;
%         sigs = [];
...
% fprintf('Saved %s using channels [%s]\n', outfile, chanTag);
%         if t == 1
%             save(fullfile(outdir, [filename '_RH_last60s_avg' num2str(i) '.mat']), 'mean_sig0');
%         else
%             save(fullfile(outdir, [filename '_LH_last60s_avg' num2str(i) '.mat']), 'mean_sig0');
%         end
%     end
% end
