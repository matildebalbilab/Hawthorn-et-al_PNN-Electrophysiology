...
%%
% pac_mean_log = log10(pac_mean + eps);  % small offset to avoid log(0)
% imagesc(phase_freqs, amp_freqs, pac_mean_log);
% title('Log-Transformed Averaged PAC');
% figure;
% imagesc(phase_freqs, amp_freqs, pac_mean_log);
% set(gca, 'YDir', 'normal');
% xlabel('Phase Freq (Hz)');
...
%%
 % Adjust this to match your original analysis
% ph = 1:0.2:30;   
% amp = 1:0.2:100;
% figure;
...
% --- Your existing figure code above this ---
...
%% Add δ–γ, θ–γ, β–γ windows to a PAC heatmap
% Assumes: x-axis = phase frequency (Hz), y-axis = amplitude frequency (Hz)

% Define bands (Hz)
%% Add PAC windows (delta–gamma, theta–gamma, beta–gamma)

% %% PAC windows (delta–gamma, theta–gamma, beta–gamma)
% 
% % Frequency bands (Hz)
% delta_band = [1 4];
% theta_band = [4 12];
...
% % ---- Delta–gamma window ----
% rectangle('Position', ...
%     [delta_band(1), gamma_amp(1), diff(delta_band), diff(gamma_amp)], ...
%     'EdgeColor','w','LineWidth',2,'LineStyle','--');
% text(mean(delta_band), gamma_amp(2) + 3, '\delta–\gamma', ...
%     'Color','w','FontSize',12, 'HorizontalAlignment','center');
% 
% % ---- Theta–gamma window ----
% rectangle('Position', ...
%     [theta_band(1), gamma_amp(1), diff(theta_band), diff(gamma_amp)], ...
%     'EdgeColor','w','LineWidth',2,'LineStyle','--');
% text(mean(theta_band), gamma_amp(2) + 3, '\theta–\gamma', ...
%     'Color','w','FontSize',12, 'HorizontalAlignment','center');
% 
% % ---- Beta–gamma window ----
% rectangle('Position', ...
%     [beta_band(1), gamma_amp(1), diff(beta_band), diff(gamma_amp)], ...
%     'EdgeColor','w','LineWidth',2,'LineStyle','--');
% text(mean(beta_band), gamma_amp(2) + 3, '\beta–\gamma', ...
%     'Color','w','FontSize',12, 'HorizontalAlignment','center');
% 
% 
% % Save
